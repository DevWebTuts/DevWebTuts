...
export default {
    ...
    computed: {
        canChangeVisibility() {
            return this.$route.name === 'user' ? this.auth.currentUser.admin || (this.auth.currentUser.id === this.$route.params.id) : (this.auth.currentUser.admin || this.$route.name === 'current_user')
        }
    },
    methods: {
